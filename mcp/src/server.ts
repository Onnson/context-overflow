 ...
 * (cross-client response routing, CVE-2026-25536).
 */
export function buildServer(): McpServer {
  ...
  server.registerTool(
    ...
    async ({ description }) => text(classifyIntent(description))
  );
  ...
  server.registerTool(
    ...
    async ({ query }) => text(findTechnique(query))
  );
  ...
  server.registerTool(
    ...
    async ({ id }) => text(applyTechnique(id))
  );
